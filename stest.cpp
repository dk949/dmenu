...
static void test(const char *path, const char *name) {
    ...
    if ((!stat(path, &st) && (FLAG('a') || name[0] != '.')                /* hidden files      */
            ...
            && (!FLAG('n') || st.st_mtime > new_.st_mtime)                /* newer than file   */
...
int main(int argc, char *argv[]) {
    ...
    ARGBEGIN {
        ...
        case 'o': /* older than file */
            file = EARGF(usage());
            if (!(FLAG(ARGC()) = !stat(file, (ARGC() == 'n' ? &new_ : &old))))
                perror(file);
            break;
        default:
        ...
        for (; argc; argc--, argv++) {
            if (FLAG('l') && (dir = opendir(*argv))) {
                ...
                while ((d = readdir(dir))) {
                    ...
                    if (r >= 0 && static_cast<size_t>(r) < sizeof path)
                        test(path, d->d_name);
                }
                closedir(dir);
            } else {
